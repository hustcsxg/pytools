...
class ModelEntry(ScheduleEntry):
    ...
    model_schedules = (
        ...
        (schedules.crontab, CrontabScheduleModel, 'crontab'),
        (schedules.schedule, IntervalScheduleModel, 'interval'),
        (schedules.solar, SolarScheduleModel, 'solar'),
    )
    ...
    def _disable(self, model):
        ...
        if self.session:
            ...
            #     obj = session.query(PeriodicTaskModel).get(model.id)
    ...
    def save(self, fields=tuple()):
        ...
        with session_cleanup(session):
            ...
            obj = session.query(PeriodicTaskModel).get(self.model.id)
        ...
        **entry sample:
        ...
        with session_cleanup(session):
            periodic_task = session.query(
                PeriodicTaskModel).filter_by(name=name).first()
            if not periodic_task:
                periodic_task = PeriodicTaskModel(name=name)
...
class DatabaseScheduler(Scheduler):
    ...
    Changes = PeriodicTaskChangedModel
    ...
    def all_as_schedule(self):
        ...
        with session_cleanup(session):
            ...
            # get all enabled PeriodicTaskModel
